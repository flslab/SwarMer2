...
class WorkerProcess(multiprocessing.Process):
    def __init__(self, count, process_id, sid, gtl, el, shared_el, results_directory, stand_by_coord):
        super(WorkerProcess, self).__init__()
        self.history = History(1)
        self.metrics = Metrics(self.history, results_directory)
        self.context = WorkerContext(count, process_id, sid, gtl, el, shared_el, self.metrics, stand_by_coord)
